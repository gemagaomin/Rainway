 ...
 * 添乘检查编辑页面
 */
public class   WorkExamineEditXCActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_work_examine_edit_xc);
        toolbar =(Toolbar) findViewById(R.id.edit_examine_toolbar);
        Intent intent=getIntent();
        dataUtil=DataUtil.getInstance();
        workPojo=(WorkPojo) intent.getSerializableExtra("work");
        if(workPojo!=null){
            toolbar.setTitle("编辑现场检查");
            isEdit=true;
        }else{
            toolbar.setTitle("添加现场检查");
    ...
    public void onNoDoubleClick(View v) {
        int id=v.getId();
        switch (id){
            case R.id.work_xc_submit_btn:
                if(Validate()){
                    final LoadingDialog loadingDialog=LoadingDialog.getInstance(WorkExamineEditXCActivity.this);
                    loadingDialog.show();
                    UserModel userModel=dataUtil.getUser();
                    workPojo.setUserId(userModel.getUserId());
                    String time=DateTimeUtil.getNewDateShow();
                    workPojo.setWorkType(DataUtil.WORK_TYPE_XCJC);
                    ...
                    new AlertDialog.Builder(WorkExamineEditXCActivity.this).setTitle("操作提示").setMessage("工作创建成功！")
                    ...
                    Toast.makeText(WorkExamineEditXCActivity.this,"区域或线路至少选择一个",Toast.LENGTH_LONG).show();
                }
                break;
            case R.id.work_examine_edit_xc_add_area:
                Intent intent=new Intent(WorkExamineEditXCActivity.this,SearchAreaActivity.class);
                startActivityForResult(intent,300);
                break;
            case R.id.work__xc_cancel_btn:
                finish();
                break;
            case R.id.work_examine_edit_xc_add_line_station:
                Intent intent1=new Intent();
                intent1.setClass(WorkExamineEditXCActivity.this,SearchLineStationActivity.class);
                intent1.putExtra("xc","xc");
                intent1.putExtra("params",(Serializable)paramsMap);
                startActivityForResult(intent1,200);
                break;
            case R.id.work_xc_edit_cancel_btn:
                EditCancel();
                break;
            case R.id.work_xc_edit_submit_btn:
                EditSubmit();
                break;
        }
    }
    ...
    private void initEdit(){
        if(isEdit){
            editBtnLinearLayout=(LinearLayout) findViewById(R.id.work_xc_edit_btn_layout);
            addBtnLinearLayout=(LinearLayout) findViewById(R.id.work_xc_btn_layout);
            editBtnLinearLayout.setVisibility(View.VISIBLE);
            addBtnLinearLayout.setVisibility(View.GONE);
            editCancel=(Button) findViewById(R.id.work_xc_edit_cancel_btn);
            editCancel.setOnClickListener(this);
            editSubmit=(Button) findViewById(R.id.work_xc_edit_submit_btn);
            editSubmit.setOnClickListener(this);
        }
    }

    private void initView(){
        lineStationLV=(ListView) findViewById(R.id.work_examine_edit_xc_line_list);
        lineAndStationAdapter=new LineStationAdapter(lineAndStationList,WorkExamineEditXCActivity.this,clickListener,R.layout.item_line_station_xc);
        lineStationLV.setAdapter(lineAndStationAdapter);
        areaLV=(ListView) findViewById(R.id.work_examine_edit_xc_area_list);
        areaAdapter=new AreaAndLineAdapter(areaList, WorkExamineEditXCActivity.this,areaListener ,R.layout.item_area_xc);
        areaLV.setAdapter(areaAdapter);
        cancel=(Button) findViewById(R.id.work__xc_cancel_btn);
        cancel.setOnClickListener(this);
        addArea=(ImageView) findViewById(R.id.work_examine_edit_xc_add_area);
        addArea.setOnClickListener(this);
        addLineStation=(ImageView) findViewById(R.id.work_examine_edit_xc_add_line_station);
        addLineStation.setOnClickListener(this);
        submit=(Button) findViewById(R.id.work_xc_submit_btn);
        submit.setOnClickListener(this);
    }

    private boolean Validate(){
        boolean res=true;
        if(lineAndStationList.size()==0&&areaList.size()==0){
            res=false;
        }
        return res;
    }
